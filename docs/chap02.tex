...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|}
        \hline
        \textbf{系统} & \textbf{Astronomy Shop} & \textbf{Hotel Reservation} & \textbf{Social Network} \\ \hline
        \textbf{代表性组件} & \texttt{frontend}、\texttt{checkout}、\texttt{payment}、\texttt{shipping} 等 & \texttt{frontend}、\texttt{search}、\texttt{reservation}、\texttt{rate} 等 & \texttt{nginx-web-server}、\texttt{compose-post-service} 等 \\ \hline
        \textbf{工作负载方式} & 使用应用内置 \texttt{load-generator} 持续发压 & \texttt{wrk2} CronJob，每分钟一次，100 RPS，50 s，指数到达 & \texttt{wrk2} CronJob，每分钟一次，10 RPS，50 s，指数到达 \\ \hline
        \textbf{设计目的} & 保留原生追踪语义，突出调用扇出与时延传播现象 & 稳定覆盖查询与预订路径，放大日志错误、超时和错误率信号 & 持续驱动发帖链路，突出级联调用、连接异常与上游暴露下游故障 \\ \hline
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{Introspector 失败类型定义与演化反馈动作}
    \label{tab:failure-analysis}
    \small
    \begin{tabularx}{\textwidth}{|l|X|X|}
        \hline
        \textbf{失败类型} & \textbf{现象描述} & \textbf{演化反馈动作} \\ \hline
        规则误导 (Rule Misleading) & 已有规则对当前场景不适用，导致误判。 & 降低规则优先级，补充负样本/反例。 \\ \hline
        错误知识检索 (Wrong Knowledge) & 检索阶段找到了相似但不正确的历史案例。 & 修正知识条目置信度，优化相似度度量。 \\ \hline
        分布外故障 (OOD) & 当前故障模式超出了现有知识库的覆盖范围。 & 触发新的离线演化流程，生成新知识。 \\ \hline
        数据不完整 (Incomplete Data) & 诊断过程中缺乏推导所需的必要证据。 & 新增观测动作或增强特定遥测采样。 \\ \hline
        超时 (Timeout) & 诊断路径效率低下，无法在规定时间内完成。 & 压缩动作链路，优化工具执行性能。 \\ \hline
    \end{tabularx}
\end{table}
